...
uint8_t wav_play_song(uint8_t *fname)
{
    ...
    if (g_audiodev.file || g_audiodev.tbuf)
    {
        ...
        {
                // 根据文件名判断动画类型（核心逻辑）
            if (strstr((char*)fname, "1")) {
             
                play_happy_emotion();  // 1.wav → 开心动画
            } else if (strstr((char*)fname, "3")) {
                
                play_sad_emotion();    // 3.wav → 难过动画
            } else {
                 
                play_standby_emotion();   // 其他文件 → 停止动画
            }
            if (wavctrl.bps == 16)
            {
